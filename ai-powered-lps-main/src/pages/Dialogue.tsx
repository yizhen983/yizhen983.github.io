        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <ol className="space-y-2 list-decimal list-inside text-muted-foreground">
              {dialogueContent.warmUp.map((q, i) => (
                <li key={i}>{q}</li>
              ))}
            </ol>
          </motion.div>
          ...
          <motion.div
            ...
            className="mb-10 p-6 rounded-2xl bg-card border border-border shadow-card"
          >
            <h2 className="font-serif text-lg font-semibold text-foreground mb-4">出現人物</h2>
            <div className="grid sm:grid-cols-2 gap-4">
              {dialogueContent.characters.map((char, i) => (
          ...
          >
            {dialogueContent.lines.map((line, index) => {
              const isZhang = line.speaker === "張輝";
              return (
                <div
                  key={index}
                  className={`flex gap-4 ${isZhang ? '' : 'flex-row-reverse'}`}
                >
                  <div className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center ${
                    isZhang ? 'bg-navy' : 'bg-secondary'
                  }`}>
                    <span className="text-primary-foreground font-serif font-bold">
                      {line.speaker.charAt(0)}
                    </span>
                  </div>
                  <div className={`flex-1 max-w-[80%] ${isZhang ? '' : 'text-right'}`}>
          ...
          >
            <Tabs defaultValue="vocabulary" className="w-full">
              <TabsList className="w-full grid grid-cols-2 mb-6">
                <TabsTrigger value="vocabulary">生詞一</TabsTrigger>
                <TabsTrigger value="grammar">語法點一</TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="grammar">
                <div className="space-y-4">
                  {dialogueGrammar.map((grammar, index) => (
                    <div 
                      key={index}
